 ...
 * @returns {string}
 */
function singlePassSubstring(details) {
    ...
    for (let i = prefixLength; i < details.url.length; i++) {
        ...
        if (c === '_') {
            cleanedUrl += details.url.substring(start, i);
            start = i + 1;
            continue;
        }
        if (c === '?' || c === '#') {
            return cleanedUrl + details.url.substring(start, i);
        }
    }
    // Append the remaining portion after the last underscore.
    return cleanedUrl + details.url.substring(start);
}

benchmark(singlePassSubstring);
